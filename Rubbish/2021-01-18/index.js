...
app.get("/api/get/select", (req, res)=> {
    const sqlSelect = "SELECT id, nombre FROM cuenta UNION ALL SELECT id, nombre FROM sub_cuenta UNION ALL SELECT id, nombre FROM auxiliar";
    db.query(sqlSelect, (err, result)=> {
        res.send(result);
    });
}); 
...
app.post("/api/insert", (req, res)=> {
    ...
    db.query(sqlInsert,  [Codigo, Descripcion, Debe, Haber], (err, result)=> {});
    db.query(sqlInsert2, [codigo, Descripcion, debe, haber], (err, result)=> {});
});
